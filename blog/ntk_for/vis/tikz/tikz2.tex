...
\node (F) [right=1.75cm of Theta] {$\mathcal{F}$};
\node (Y) [right=1.5cm of F] {$Y$};
\node (TX) [above=1.5cm of X] {$\mathcal{T}_{x} \X$};
\node (TTheta) [above=1.5cm of Theta] {$\mathcal{T}_{\theta} \Theta$};
\node (TF) [above=1.5cm of F] {$\mathcal{T}_{f_\theta} \mathcal{F}$};
\node (TY) [above=1.5cm of Y] {$\mathcal{T}_{y} Y$};
...
\draw[->, dotted] (F) to node[left] {$f_\theta \in \mathcal{F}$} (TF);
\draw[->] (F) to node[above] {ev$_x$} (Y);
\draw[->] (TF) to node[below] {$d_{f_\theta}$ev$_x$} (TY);
\draw[->] (X) to[bend right=30] node[below] {\(f_\theta\)} (Y);
